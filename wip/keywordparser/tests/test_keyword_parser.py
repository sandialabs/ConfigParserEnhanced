...
def test_invalid_supported_envs_filename_raises():
    ...
    assert ("ERROR: Unable to load configuration .ini file" in exception_message)
...
def test_values_do_not_contain_whitespace_or_delimiter(multiple_values):
    ...
    assert (f"The following valu{es} contain{s} whitespace or the delimiter" " '_':" in exc_msg)
    assert "- intel 18\n" in exc_msg
...
def test_values_are_unique():
    ...
    print(exc_msg)
    assert (
        "Values for 'bad_config.ini'['machine-type-1']['intel-18.0.5-mpich-7.7.15'] "
        "contains duplicates:"
    ) in exc_msg
...
def test_general_value_matches_correct_key(general_section_order):
    ...
    other_section = ("intel-19.0.4-mpich-7.7.15:\n" "    intel-19")
    config = "\n".join(
        [
            "[machine-type-1]",
...
def test_matched_value_not_in_config_section_raises():
    ...
    assert ("ERROR:  Unable to find value 'bad_value' in values for " "'machine-type-1'") in exc_msg
...
def test_get_msg_for_list_works_correctly():
    ...
    msg = kp.get_msg_for_list("Message here.", item_list, extras="\nExtra message here.")
