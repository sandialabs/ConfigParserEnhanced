...
def test_supported_envs_ini_is_read_correctly():
    ekp = EnvKeywordParser("default-env", "machine-type-5", "test_supported_envs.ini")
    ...
    e.g.
        [machine-type-5]
        intel-18.0.5-mpich-7.7.6:   << se["machine-type-5"].keys()
            intel-18             <-|
            intel                  |-< se["machine-type-5"].values()
            default-env          <-|
        intel-19.0.4-mpich-7.7.6:   << se["machine-type-5"].keys()
            intel-19                << se["machine-type-5"].values()
    """
    assert [_ for _ in se.keys()] != ["DEFAULT"]
    assert [_ for _ in se["machine-type-5"].keys()] != []
    assert "default-env-hsw" in ekp.get_aliases()


def test_invalid_supported_envs_filename_raises():
    with pytest.raises(IOError) as excinfo:
        ekp = EnvKeywordParser("", "machine-type-5", "invalid_filename_here.ini")
        ekp.supported_envs
...
@pytest.mark.parametrize("keyword", [
    {
        "str": "machine-type-5-intel-19.0.4_mpich-7.7.15_hsw_openmp_static_dbg",
        "qualified_env_name": "machine-type-5-intel-19.0.4-mpich-7.7.15-hsw-openmp",
        "system_name": "machine-type-5",
    },
    {
        "str": "default-env-knl",
        "qualified_env_name": "machine-type-5-intel-19.0.4-mpich-7.7.15-knl-openmp",
        "system_name": "machine-type-5",
    },
    {
        "str": "intel_hsw",
    ...
    },
    {
        ...
        "qualified_env_name": "machine-type-3-arm-20.1-openmpi-4.0.3-openmp",
        "system_name": "machine-type-3",
    },
    {
        "str": "arm-serial",
        "qualified_env_name": "machine-type-3-arm-20.0-openmpi-4.0.2-serial",
        "system_name": "machine-type-3",
    },
])
...
def test_underscores_hyphens_dont_matter_for_kw_str(kw_str):
    ekp = EnvKeywordParser(kw_str, "machine-type-5", "test_supported_envs.ini")
    assert ekp.qualified_env_name == "machine-type-5-intel-19.0.4-mpich-7.7.15-hsw-openmp"


def test_nonexistent_env_name_or_alias_raises():
    ekp = EnvKeywordParser("bad_kw_str", "machine-type-5", "test_supported_envs.ini")
...
@pytest.mark.parametrize("inputs", [
    ...
    {"system_name": "machine-type-5", "build_name": "intel-19-mpich-7.2",
     "unsupported_components": ["intel-19", "mpich-7.2"]},
    {"system_name": "machine-type-3", "build_name": "arm-20.2",
     "unsupported_components": ["arm-20.2"]},
    {"system_name": "machine-type-3", "build_name": "arm-20.1-openmpi-4.0.2",
...
def test_unsupported_versions_are_rejected(inputs):
    ...
    if inputs["system_name"] == "machine-type-5":
...
@pytest.mark.parametrize("inputs", [
    {"system_name": "machine-type-5", "build_name": "intel-hsw-serial",
     "unsupported_component": "serial"},
    {"system_name": "machine-type-5", "build_name": "intel-serial",
     "unsupported_component": "serial"},
    {"system_name": "test-system", "build_name": "env-name-openmp",
     "unsupported_component": "openmp"},
    {"system_name": "machine-name-1", "build_name": "cuda-serial",
     "unsupported_component": "static"},
    {"system_name": "machine-name-1", "build_name": "cuda-10-openmp",
     "unsupported_component": "openmp"},
])
def test_unsupported_node_types_are_rejected(inputs):
    ...
    if inputs["system_name"] == "machine-name-1":
...
def test_underscores_hyphens_dont_matter_for_aliases():
    # "intel-18" and "intel_default" are aliases for "machine-type-5"
    ekp = EnvKeywordParser("intel-18", "machine-type-5", "test_supported_envs.ini")
...
@pytest.mark.parametrize("bad_alias", [
    {
        "alias": "intel",
        "err_msg": "ERROR:  Aliases for 'machine-type-5' contains duplicates:",
    },
    {
        "alias": "intel-18.0.5-mpich-7.7.15",
        "err_msg": ("ERROR:  Alias found for 'machine-type-5' that matches an environment"
                    " name:"),
    },
    {
...
def test_alias_values_are_unique(bad_alias):
    ...
    with pytest.raises(SystemExit) as excinfo:
        EnvKeywordParser("default-env", "machine-type-5", filename)
    exc_msg = excinfo.value.args[0]
...
def test_alias_values_do_not_contain_machine-name-4space(multiple_aliases):
    bad_supported_envs = (
        "[machine-type-5]\n"
        "intel-18.0.5-mpich-7.7.15: # Comment here\n"
    ...
    with pytest.raises(SystemExit) as excinfo:
        EnvKeywordParser("intel-18", "machine-type-5", filename)
    exc_msg = excinfo.value.args[0]

    es = "es" if multiple_aliases is True else ""
    s = "" if multiple_aliases is True else "s"
    assert f"The following alias{es} contain{s} machine-name-4space:" in exc_msg
    assert "- intel 18\n" in exc_msg
...
def test_flag_without_any_options_raises():
    """
    Ensure we never have a situation like the following::
        ...
        # etc.
    """
    ckp = ConfigKeywordParser("build-name", "test_supported_envs.ini")
    msg = ckp.get_msg_showing_supported_options(
