...
def test_argument_parser_functions_correctly(data):
    ...
    assert (
        ...
        == Path(
            ...
            if data["supported_sys_expected"] == "default"
            else data["supported_sys_expected"]
        ).resolve()
    )
    assert (
        ...
        == Path(
            ...
            if data["supported_envs_expected"] == "default"
            else data["supported_envs_expected"]
        ).resolve()
    )
    assert (
        le.args.environment_specs_file
        == Path(
            le.load_env_config_data["load-env"]["environment-specs"]
            if data["environment_specs_expected"] == "default"
            else data["environment_specs_expected"]
        ).resolve()
    )
...
def test_load_env_ini_file_used_if_nothing_else_explicitly_specified():
    ...
    assert (
        le.args.supported_systems_file
        == Path(le.load_env_config_data["load-env"]["supported-systems"]).resolve()
    )
    assert (
        le.args.supported_envs_file
        == Path(le.load_env_config_data["load-env"]["supported-envs"]).resolve()
    )
    assert (
        le.args.environment_specs_file
        == Path(le.load_env_config_data["load-env"]["environment-specs"]).resolve()
    )
