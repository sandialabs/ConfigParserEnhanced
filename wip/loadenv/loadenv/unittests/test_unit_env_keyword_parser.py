...
def test_env_keyword_parser_matches_correctly(keyword):
    ekp = EnvKeywordParser(
        keyword["str"], keyword["system_name"], "test_supported_envs.ini"
    )
...
def test_nonexistent_env_name_or_alias_raises():
    ...
    assert (
        "ERROR:  Unable to find alias or environment name for system " "'machine-type-1' in"
    ) in exc_msg
...
@pytest.mark.parametrize(
    ...
    [
        ...
        },
        {
            ...
            "err_msg": (
                "ERROR:  Alias found for 'machine-type-1' that matches an " "environment name:"
            ),
        },
        {
